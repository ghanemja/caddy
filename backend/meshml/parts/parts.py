...
class PartInfo:
    ...
    name: Optional[str] = (
        None  # human-assigned name, e.g. "backrest", "wheel_front_left"
    )
...
class PartTable:
    ...
    vertex_part_labels: (
        np.ndarray
    )  # [N] array of integer labels (same N as vertex count)
...
def infer_shape_hint(extents: np.ndarray, threshold_ratio: float = 0.3) -> str:
    ...
    if (
        min_extent / max_extent < threshold_ratio
        and mid_extent / max_extent < threshold_ratio
    ):
        return "long_thin"
...
def build_part_table_from_segmentation(
    ...
    epsilon: float = 0.01,
    preliminary_names: Optional[Dict[int, str]] = None,
) -> PartTable:
    """
    ...
    Args:
        ...
        preliminary_names: Optional dict mapping part_id -> preliminary name from segmentation
    ...
    if len(vertices) != len(part_labels):
        raise ValueError(
            f"Vertices and labels must have same length: {len(vertices)} != {len(part_labels)}"
        )
    ...
    for part_id in unique_part_ids:
        ...
        # Get preliminary name if available
        preliminary_name = None
        if preliminary_names and int(part_id) in preliminary_names:
            preliminary_name = preliminary_names[int(part_id)]

        # Create PartInfo
        part_info = PartInfo(
            part_id=int(part_id),
            name=preliminary_name,  # Set as name so it shows up in UI
            centroid=centroid,
            bbox_min=bbox_min,
            bbox_max=bbox_max,
...
def part_table_to_labeling_json(
    ...
    for part_id in sorted_part_ids:
        ...
        # Use preliminary name from segmentation as provisional_name if name exists
        # Otherwise use part_X as fallback
        provisional_name = (
            part_info.name if part_info.name else f"part_{part_info.part_id}"
        )

        part_entry = {
            "part_id": int(part_info.part_id),
            "provisional_name": provisional_name,
            "name": part_info.name,  # This will be None for preliminary names, set by user later
            ...
            "approx_volume": (
                float(part_info.approx_volume)
                if part_info.approx_volume is not None
                else None
            ),
            "approx_area": (
                float(part_info.approx_area)
                if part_info.approx_area is not None
                else None
            ),
        }
    ...
    for part_entry in labels_json["parts"]:
        ...
        if "description" in part_entry:
            part_info.description = (
                str(part_entry["description"]).strip()
                if part_entry["description"]
                else None
            )
