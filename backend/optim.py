...
# ----------------- FreeCAD bootstrap -----------------
# Load FreeCAD BEFORE importing CadQuery, so we can initialize CadQuery with it
...
def load_freecad():
    ...
    if conda_prefix:
        ...
        if os.path.exists(conda_freecad_path):
            try:
                # Add conda lib to library path for FreeCAD dependencies
                conda_lib = os.path.join(conda_prefix, "lib")
                if conda_lib not in sys.path:
                    sys.path.append(conda_lib)
                
                # Set DYLD_LIBRARY_PATH for macOS (if not already set)
...
# Make sure you're using Python from the conda environment where FreeCAD is installed
FreeCAD = None
try:
...
# Paths - optim.py is in backend/, assets are at root level
BACKEND_DIR = os.path.dirname(__file__)  # backend/ directory
ROOT_DIR = os.path.dirname(BACKEND_DIR)  # Root of project
BASE_DIR = ROOT_DIR  # For backward compatibility
ASSETS_DIR = os.path.join(ROOT_DIR, "frontend", "assets")
os.makedirs(ASSETS_DIR, exist_ok=True)
ROVER_GLB_PATH = os.path.join(ASSETS_DIR, "rover.glb")
# Only use cqparts if FreeCAD is available
USE_CQPARTS = os.environ.get("USE_CQPARTS", "1") == "1" and FreeCAD is not None

# Get paths - optim.py is in backend/, frontend is in ../frontend/
...
# ADAPTER is only created if Rover is available
# ADAPTER is only created if Rover is available
ADAPTER = ModelAdapter(Rover) if Rover is not None else None if Rover is not None else None
...
register_component(
    ...
    ComponentSpec(
        cls=SensorFork,
        add_fn=lambda adapter, **p: ADAPTER.add("sensor_fork", **p) if ADAPTER is not None else None,
        param_map={
            "width_mm": "width",
            "depth_mm": "depth",
            "height_mm": "height",
...
# Register rover component only if Rover class is available
...
def _baseline_cqparts_source(max_chars: int = 20000) -> str:
    ...
    # Try class-by-class extraction (optional - only if Rover is available)
    if not chunks:
        print("[baseline_source] Trying class-by-class extraction...")
        try:
            if Rover is not None:
                from app.models.cad.robot_base import RobotBase
                ...
                print("[baseline_source] ⚠ Rover not available, skipping")
        except Exception as e:
            print(f"[baseline_source] ⚠ Class extraction failed (optional): {e}")
...
def build_rover_scene_glb_cqparts(RoverClass=None) -> bytes:
    ...
    if RoverClass is None:
        raise RuntimeError("Rover class not available. Cannot build rover scene.")
...
def build_rover_scene_glb(_: Optional[Dict[str, Any]] = None) -> bytes:
    ...
    if os.path.exists(ROVER_GLB_PATH):
        with open(ROVER_GLB_PATH, "rb") as f:
            return f.read()
    raise FileNotFoundError("cqparts disabled, and frontend/assets/rover.glb not found")
...
def demo_mars_rover():
    ...
    return Response(
        "Demo image not found. Please save the Mars rover image to frontend/assets/demo/rover.png or frontend/assets/demo/mars_rover.jpg",
        status=404,
        mimetype="text/plain"
    )
