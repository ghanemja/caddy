...
class OtherController(cqparts.Part):
    """Simple placeholder for a control board."""
    width = PositiveFloat(65)
    depth = PositiveFloat(55)
    height = PositiveFloat(12)
    _render = render_props(color=(100, 80, 60, 255))

    def make(self):
        return cq.Workplane("XY").box(self.width, self.depth, self.height, centered=(True, True, False))
...
class PCBBoard(cqparts.Part):
    ...
    def mount_points(self, offset=0):
        ...
        h = wp.rect(self.hole_length, self.hole_width, forConstruction=True).vertices()
        return h
...
class BeagleBoneBlack(PCBBoard):
    ...
    def make(self):
        ...
        holes = (
            self.mount_points(offset=self.thickness * 2)
            .circle(self.hole_size / 2)
            .extrude(-self.thickness * 4)
        )
        board = board.cut(holes)
        return board
