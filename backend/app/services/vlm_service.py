...
def load_finetuned_model():
    ...
    try:
        ...
        try:
            ...
            print(
                "[vlm]   or: pip uninstall scipy && conda install -c conda-forge scipy"
            )
            print(
                "[vlm] Continuing anyway - scipy may not be needed for model loading..."
            )
        ...
        print(
            "[vlm] Install with: pip install transformers peft torch pillow accelerate"
        )

        # Check if it's a scipy/BLAS issue
        if (
            "scipy" in error_msg.lower()
            or "NEWLAPACK" in error_msg
            or "_fblas" in error_msg
        ):
            print(
            ...
            print(
                "[vlm]   1. Reinstall scipy with conda: conda install -c conda-forge scipy openblas"
            )
            print(
                "[vlm]   2. Or use pip with openblas: pip uninstall scipy && pip install scipy"
            )
            print(
                "[vlm]   3. Or disable fine-tuned model: export USE_FINETUNED_MODEL=0"
            )
        ...
        if (
            "NEWLAPACK" in error_msg
            or "_fblas" in error_msg
            or "scipy" in error_msg.lower()
        ):
            print(f"[vlm] ✗ scipy BLAS compatibility error: {e}")
            print("[vlm] This is a known macOS issue. To fix:")
            print(
                "[vlm]   conda install -c conda-forge scipy openblas --force-reinstall"
            )
...
def stitch_images_side_by_side(img1_data_url: str, img2_data_url: str) -> str:
    ...
    try:
        ...
        if h1 > target_height or h2 > target_height:
            ...
            img1 = img1.resize(
                (int(w1 * scale1), target_height), Image.Resampling.LANCZOS
            )
            img2 = img2.resize(
                (int(w2 * scale2), target_height), Image.Resampling.LANCZOS
            )
        elif h1 != h2:
            # Make same height
            target_h = min(h1, h2)
            if h1 > target_h:
                scale = target_h / h1
                img1 = img1.resize(
                    (int(w1 * scale), target_h), Image.Resampling.LANCZOS
                )
            if h2 > target_h:
                scale = target_h / h2
                img2 = img2.resize(
                    (int(w2 * scale), target_h), Image.Resampling.LANCZOS
                )
        ...
        print(
            f"[stitch] ✓ Combined {img1.width}x{img1.height} + {img2.width}x{img2.height} → {combined.width}x{combined.height}"
        )
...
def call_vlm(
    final_prompt: str,
    image_data_urls: Optional[Union[List[str], str]],
    *,
    ...
    system_prompt = vlm_system_prompt or (
        get_codegen_prompt() if not expect_json else get_system_prompt()
    )
    ...
    if _finetuned_model is not None and _finetuned_processor is not None:
        try:
            ...
            inputs = _finetuned_processor(
                images=images if images else None, text=prompt_text, return_tensors="pt"
            )
            ...
            if device == "cpu":
                ...
                print(
                    f"[vlm]   ⚠ CPU detected: Reduced max_tokens to {max_tokens} for faster generation"
                )
            else:
            ...
            print(
                f"[vlm]   Input shape: {inputs['input_ids'].shape if 'input_ids' in inputs else 'N/A'}"
            )
            print(f"[vlm]   Device: {device}")
            print(
                f"[vlm]   Starting generation (this may take 30-120 seconds on CPU)..."
            )
                ...
                while not generation_done.is_set():
                    ...
                    if elapsed < warning_threshold:
                        ...
                        if not generation_done.is_set():
                            print(
                                f"[vlm]   Still generating... ({elapsed:.0f}s elapsed)",
                                flush=True,
                            )
                    else:
                        ...
                        if not generation_done.is_set():
                            elapsed = time.time() - start_time
                            print(
                                f"[vlm]   ⚠ Still generating after {elapsed:.0f}s (may take 2-5 minutes on CPU)...",
                                flush=True,
                            )
            ...
            elapsed = time.time() - start_time
            print(
                f"[vlm]   ✓ Generation completed in {elapsed:.1f} seconds", flush=True
            )
            ...
            response = _finetuned_processor.decode(
    ...
    if OLLAMA_URL:
        try:
            ...
            timeout_seconds = (
                300 if not expect_json else 120
            )  # 5 min for code, 2 min for JSON

            print(
                f"[vlm] Sending to Ollama (timeout: {timeout_seconds}s, context: {payload['options']['num_ctx']})"
            )
            ...
            r = requests.post(
                f"{OLLAMA_URL}/api/generate", json=payload, timeout=timeout_seconds
            )
    ...
    if LLAVA_URL:
        try:
            current_system_prompt = codegen_prompt if not expect_json else system_prompt
            payload = {"prompt": current_system_prompt + "\n\n" + final_prompt}
