...
class FinetunedVLMClient:
    ...
    def __init__(self):
        ...
        # Import the VLM function from app.services.vlm_service (new location)
        # Fallback to optim.py for legacy compatibility
        try:
            # Try importing from app.services.vlm_service first (new location)
            from app.services.vlm_service import call_vlm, load_finetuned_model

            self._call_vlm = call_vlm
            self._load_finetuned_model = load_finetuned_model
            self._available = True
            print("[FinetunedVLMClient] Using call_vlm from app.services.vlm_service")
        except ImportError:
            # Fallback to optim.py (legacy)
            try:
                import sys
                import os
                ...
                self._load_finetuned_model = load_finetuned_model
                self._available = True
                print("[FinetunedVLMClient] Using call_vlm from optim.py (legacy)")
            except ImportError as e:
                print(
                    f"[FinetunedVLMClient] Warning: Could not import call_vlm from app.services.vlm_service or optim.py: {e}"
                )
                print("[FinetunedVLMClient] Client will not be available.")
        ...
        if images:
            ...
            for img in images:
                try:
                    ...
                    print(
                        f"[FinetunedVLMClient] Warning: Failed to load image {img.path}: {e}"
                    )
                    continue
