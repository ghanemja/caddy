...
# Add XPart/partgen to path (use absolute path to avoid issues)
# IMPORTANT: Insert at position 0 so utils.misc is found before P3-SAM/utils
_xpart_partgen_path = os.path.join(os.path.dirname(__file__), "..", "XPart/partgen")
_xpart_partgen_path = os.path.abspath(_xpart_partgen_path)
if _xpart_partgen_path not in sys.path:
    sys.path.insert(
        0, _xpart_partgen_path
    )  # Insert at 0 so XPart/partgen/utils takes precedence
...
def build_P3SAM(self):  # build p3sam
    ######################## Sonata ########################
    # Use a writable location for sonata download
    import os

    download_root = os.path.join(os.path.expanduser("~"), ".cache", "sonata")
    os.makedirs(download_root, exist_ok=True)
    self.sonata = sonata.load(
        "sonata", repo_id="facebook/sonata", download_root=download_root
    )
...
def load_state_dict(
    ...
    if ckpt_path is not None:
        ...
        ckpt_path = hf_hub_download(
            ...
            local_dir=os.path.join(os.path.expanduser("~"), "/.cache/p3sam/weights"),
        )
