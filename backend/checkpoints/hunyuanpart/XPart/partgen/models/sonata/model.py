...
def load(
    ...
    # Disable flash attention if not available (avoids requiring flash-attn installation)
    try:
        import flash_attn

        flash_available = True
    except ImportError:
        print("[Sonata] Warning: flash-attn not installed, disabling flash attention")
        flash_available = False
        ckpt["config"]["enable_flash"] = False

    # Also disable flash in all nested configs (Block, SerializedAttention, etc.)
    if not flash_available:

        def disable_flash_recursive(config):
            if isinstance(config, dict):
                if "enable_flash" in config:
                    config["enable_flash"] = False
                for v in config.values():
                    disable_flash_recursive(v)
            elif isinstance(config, (list, tuple)):
                for item in config:
                    disable_flash_recursive(item)

        disable_flash_recursive(ckpt["config"])
