...
class RobotBase(Lasercut):
    ...
    def mate_back(self, offset=5):
        return Mate(self, CoordSystem(
            origin=(-self.length/2 + offset, 0, self.thickness), xDir=(1,0,0), normal=(0,0,1)
        ))
    ...
    def mate_RL(self, offset=0):
        ...
        return Mate(self, CoordSystem(
            origin=(-self.length/2 + offset,  self.width/2, 0), xDir=(1,0,0), normal=(0,0,-1)
        ))
...
class Rover(cqparts.Assembly):
    ...
    def _axle_offsets(self):
        ...
        if n == 1:
            return [self.length/2 - self.chamfer]   # single axle near the front chamfer
    ...
    def make_components(self):
        base = RobotBase(length=self.length, width=self.width,
                         chamfer=self.chamfer, thickness=self.thickness)
        ...
        for i, off in enumerate(offsets):
            comps[f"Ldrive_{i}"] = MountedStepper(stepper=self.stepper, driven=self.wheel, target=base)
            comps[f"Rdrive_{i}"] = MountedStepper(stepper=self.stepper, driven=self.wheel, target=base)
    ...
    def make_constraints(self):
        c = [
            ...
            Coincident(self.components["electronics"].mate_origin, self.components["base"].mate_back()),
            Coincident(self.components["sensors"].mate_front(),  self.components["base"].mate_front()),
        ]
        # Constrain each axle pair to rails at its offset
        offsets = self._axle_offsets()
        for i, off in enumerate(offsets):
            c += [
                Coincident(self.components[f"Ldrive_{i}"].mate_corner(flip= 1), self.components["base"].mate_RL(offset=off)),
                Coincident(self.components[f"Rdrive_{i}"].mate_corner(flip=-1), self.components["base"].mate_RR(offset=off)),
            ]
        return c