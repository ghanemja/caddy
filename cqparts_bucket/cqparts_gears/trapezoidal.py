...
class TrapezoidalGear(Gear):
    ...
    face_angle = PositiveFloat(
        30, doc="angle of tooth edge radial edge (unit: degrees)"
    )
    spacing_ratio = FloatRange(
        0, 1, 0.5, doc="tooth thickness as a ratio of the distance between them"
    )
    ...
    def initialize_parameters(self):
        ...
        if self.tooth_height is None:
            self.tooth_height = (
                0.2 * self.effective_radius
            )  # default: 20% effective radius
    ...
    def _make_tooth_template(self):
        ...
        tooth = cadquery.Workplane("XY", origin=(0, 0, -self.width / 2)).moveTo(
            side_tangent_radius * cos(side_angle), side_tangent_radius * sin(side_angle)
        )
        opposite_point = (
        ...
        if self.face_angle:
            ...
            # )
        tooth = tooth.lineTo(
            -cos(extra_side_angle) * outer_radius, sin(extra_side_angle) * outer_radius
        )
        opposite_point = (
    ...
    def make(self):
        ...
        gear = (
            cadquery.Workplane("XY", origin=(0, 0, -self.width / 2))
            .circle(inner_radius)
            .extrude(self.width)
        )
        ...
        period_arc = 360.0 / self.tooth_count
